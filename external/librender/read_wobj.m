...
for iline=1:length(ftype)
    ...
    switch(type)
        ...
        case('v') % vertices
            ...
            % new added vertice color
            elseif (length(data) == 6)
                 % Reserve block of memory
                if(mod(nv,10000)==1), vertices(nv+1:nv+10001,1:3)=0; end
                % Add to vertices list X Y Z
                vertices(nv,1:3)=data(1:3);
                vertices_color(nv,1:3)=data(4:6);
                nvc = nv;
            else
                % Reserve block of memory
        ...
        case{'#','$'}
            ...
            if ~isempty(findstr(tline,'% File units = '))
              OBJ.units = tline(18:end);
            else
              OBJ.units = 'm'; 
            end
...
OBJ.vertices_color = vertices_color(1:nvc,:);
